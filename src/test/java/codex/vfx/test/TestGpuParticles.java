...
public class TestGpuParticles extends DemoApplication {
    ...
    public static void main(String[] args) {
        ...
        app.setLightProbeEnabled(false);
    ...
    public void demoInitApp() {
        ...
        System.out.println("GpuParticleTest: creating " + (particleMapSize * particleMapSize) + " particles using "
        ...
        if (useImageParticles) {
            ...
            program.build();
            program.register();
            GpuImageParticleGeometry geometry = new GpuImageParticleGeometry(clContext, clQueue, particleMapSize,
                    particleMapSize);
            ...
            Program program = clContext.createProgramFromSourceFiles(assetManager,
                    "Shaders/GpuParticleComputeBuffer.cl");
            program.build();
            program.register();
            GpuBufferParticleGeometry geometry = new GpuBufferParticleGeometry(clContext, clQueue,
                    particleMapSize * particleMapSize);
            geometry.setKernels(program, "initParticleData", "updateParticleData");
    ...
    public class GpuBufferParticleGeometry extends CLDrivenGeometry {
        ...
        protected void initOpenGLResources() {
            ...
            MeshUtils.initializeVertexBuffer(mesh, VertexBuffer.Type.Position, VertexBuffer.Usage.Stream,
                    VertexBuffer.Format.Float, pb, 3);
            // color buffer
            FloatBuffer cb = BufferUtils.createFloatBuffer(capacity * 4);
            VertexBuffer b = MeshUtils.initializeVertexBuffer(mesh, VertexBuffer.Type.Color, VertexBuffer.Usage.Stream,
                    VertexBuffer.Format.Float, cb, 4);
        }

        @Override
        protected void initOpenCLResources() {
            posBuf = context.bindVertexBuffer(mesh.getBuffer(VertexBuffer.Type.Position), MemoryAccess.READ_WRITE);
            clrBuf = context.bindVertexBuffer(mesh.getBuffer(VertexBuffer.Type.Color), MemoryAccess.READ_WRITE);
