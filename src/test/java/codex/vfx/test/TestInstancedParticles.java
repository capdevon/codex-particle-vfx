...
import codex.vfx.test.util.DemoStyles;
...
public class TestInstancedParticles extends DemoApplication implements ParticleDriver<ParticleData> {
    ...
    private VersionedReference<Double> rateRef, speedRef, impulseRef, lifeRef, airRef, angImpRef, chunkRef, gravityRef;

    public static void main(String[] args) {
        TestInstancedParticles app = new TestInstancedParticles();
    ...
    public void demoInitApp() {
        ...
        main.setLocalTranslation(10, settings.getHeight() - 10, 0);
    ...
    public void updateGroup(ParticleGroup group, float tpf) {
        time += tpf;
        if (time > rateRef.get().floatValue()) {
            for (int i = 0, n = chunkRef.get().intValue(); i < n; i++) {
                ...
                p.setScale(.4f);
                p.color = new Range(ColorRGBA.Black, ColorRGBA.randomColor(), Interpolator.Color, Easing.inCubic);
    ...
    private VersionedReference<Double> createSlider(Container c, int row, String label, float value, float min,
            float max) {
        Slider s = new Slider();
