...
public interface ParticleDriver<T extends Particle> {
    ...
    /**
     * Performs an update on a particle belonging to the particle group being
     * driven.
     * 
     * @param particle
     * @param tpf
    ...
    public static final ParticleDriver Rotation = new ParticleDriver<ParticleData>() {
        ...
        public void updateParticle(ParticleData particle, float tpf) {
            particle.getRotation().multLocal(new Quaternion().fromAngles(
                    particle.angularVelocity.x * tpf,
                    particle.angularVelocity.y * tpf, 
                    particle.angularVelocity.z * tpf));
        }
     ...
     * <p>
     * If the direction vector is null, then the group's world rotation will be used
     * to determine the direction of the impulse.
     * <p>
    ...
    public static class DirectionalImpulse implements ParticleDriver<ParticleData> {
        ...
        public void particleAdded(ParticleGroup group, ParticleData p) {
            VfxUtils.offsetByAngle(getDirection(group).multLocal(magnitude), VfxUtils.gen.nextFloat(-angle, angle),
                    p.linearVelocity);
        }
