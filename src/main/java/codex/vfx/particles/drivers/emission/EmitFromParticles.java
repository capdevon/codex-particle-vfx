...
public class EmitFromParticles implements ParticleDriver<ParticleData> {
    ...
    public void particleAdded(ParticleGroup group, ParticleData p) {
        if (spawner == null && !emitter.isEmpty()) {
            ...
            p.setPosition(
                    group.getVolume().getNextPosition(emitter.get(VfxUtils.gen.nextInt(emitter.size())).transform));
        }
    }
    ...
    /**
     * Sets this driver to stop emitting particles if the target group is full.
     * <p>
     * default=true
     * 
     * @param stopIfFull
